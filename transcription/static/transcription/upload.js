...
dropZone.addEventListener("dragleave", () => {
  ...
  dropZoneText.textContent =
    "Drag & drop your audio here, or click to select a file";
});
...
function handleFiles(files) {
  ...
  function isValidAudio(file) {
    ...
    const allowedExtensions = [".mp3", ".wav", ".m4a", ".aac", ".mpeg"];

    // Try MIME type
    if (file.type && allowedMimeType.includes(file.type)) {
      return true;
    }
...
function createUploadItem(file) {
  ...
  barWrap.className =
    "w-full bg-gray-200 rounded-full h-2 mt-2 overflow-hidden";
